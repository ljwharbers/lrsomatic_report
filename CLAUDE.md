...
| `circos_bnd.R` | The breakend circos in the SV section: `bnd_links()` picks the drawable rearrangements, `bnd_panel_genes()` the panel genes to label, `draw_bnd_circos()` draws and returns it as *inline* SVG with per-arc ids |
...
- **The breakend circos is inline SVG tagged by sentinel colour, and it is the one plot the page can touch.** `templates/sections/_sv.qmd` draws a second, BND-only circos over just the chromosomes a breakend touches, and clicking a row in the SV table highlights that row's arc. Three things make that work, and each is load-bearing:
  - **It is inlined, not base64'd.** `draw_circos()`'s output is embedded as `<img src="data:image/svg+xml;base64,…">`, a *replaced element* whose internals no page script or stylesheet can reach — even ids inside it would be unreachable. `draw_bnd_circos()` therefore returns SVG *markup*, handed to `HTML()`. The upside is that the plot inherits page CSS; the cost is that svglite's `<defs><style>` block becomes **global page CSS** once inlined, so `.svglite` is renamed to `.bnd-svglite` on the way out or its `polyline { stroke: #000000 }` rule leaks into the document.
  - **Objects are identified by colour, not by document order.** svglite emits no ids or classes, so every arc, chromosome label, gene label, gene body and connector line is *drawn* in a colour unique to it (`#10xxxx` … `#50xxxx`, always decimal digits so no hex-case handling is needed), and `.tag_bnd_svg()` rewrites the SVG text: it looks each colour up in a registry — so a cytoband grey that merely matches the pattern, like `#101010`, is left alone — then attaches `class`/`data-*` and **strips the inline `style` off shapes** so CSS owns stroke, width and opacity. Order-based tagging was rejected because connector lines and BND arcs are both `<polyline>` and would have to be told apart by position. This is also why the SCSS rules must stay nested under `.bnd-circos`: svglite's own `.bnd-svglite polyline` rule is specificity (0,1,1) and beats a bare `.bnd-link`.
  - **Quarto lowercases the SVG's attribute names, and the plot has to repair `viewBox` itself.** Inlined markup goes through Pandoc and Quarto's self-contained post-processing, which normalise attribute case: svglite's `viewBox` arrives as `viewbox`, `textLength`/`lengthAdjust` as lowercase, `xmlns:xlink` as `xlink`. A `{=html}` raw block does **not** avoid this — verified, both paths come out identical. HTML parsers are supposed to case-correct SVG attributes on the way in, but SVG attributes are case-sensitive once parsed, and a plot with no `viewBox` has no aspect ratio, so `height: auto` cannot scale it. The section's script therefore copies `viewbox` to `viewBox` and drops the intrinsic `pt` width/height; it is a no-op where the parser already did the right thing. Losing `textLength` only means labels render at their natural width. This is also why a standalone rasteriser (`rsvg-convert`) shows the plot mis-sized while a browser does not — don't debug from that.
  - **A tagging failure disables highlighting rather than lying.** `draw_bnd_circos()` checks that every arc it drew was tagged exactly once and that each gene label renders the symbol it was paired with; on mismatch it returns `interactive = FALSE`, and the template shows the plot as a plain figure with a notice. A plot that highlights the *wrong* arc is worse than one that does not highlight.

  Two rules about *what* it draws. Arcs are `svclass ∈ {translocation, intra-chr breakend}` with both loci mapped — intra-chromosomal breakends are included so that every BND row a user can click has an arc, and a single breakend has no partner locus so it necessarily has none. **The gene track is the `panel_hit` column's genes**: R draws the superset (every coordinate-carrying panel's genes within `SV_PANEL_WINDOW_BND` of a drawn breakend, via the same `foverlaps()` test as `sv_panel_hits()`), and the client shows only the genes named by currently-visible rows. Two consequences that look like bugs but are not: under the default `--gene-panel none` no row has a panel hit, so **the gene track is empty on load**; and a symbol-only or custom panel produces `panel_hit` labels but no gene bodies, because it carries no coordinates. Gene bodies have a minimum angular width — MYC is 7 kb against a 145 Mb sector — so they are position markers, not spans to scale. Selection state lives in the section's own script rather than DT's (`selection = "none"`): `Scroller` + `deferRender` destroy `<tr>` nodes on scroll, and it is re-applied from a `Set` of SV ids on every `draw.dt`.
